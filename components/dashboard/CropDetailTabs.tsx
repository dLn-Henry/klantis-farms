...
import type { CropCycle } from "@/lib/data/mock/crop-cycles";

const TABS = ["Overview", "Activities", "Timeline"] as const;
type Tab = (typeof TABS)[number];

export function CropDetailTabs({ cycle }: { cycle: CropCycle }) {
      ...
      {tab === "Overview" && (
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            ["Field", cycle.field],
            ["Season", cycle.season],
            ["Planting", cycle.plantingDate],
            ["Expected Harvest", cycle.expectedHarvest],
            ["Area", cycle.area],
            ["Status", cycle.status],
          ].map(([label, value]) => (
      ...
      {tab === "Activities" && (
        <div className="bg-white border border-border rounded-md overflow-hidden">
          {cycle.events.length === 0 && (
            <p className="p-5 text-sm text-ink-soft">No activities recorded yet.</p>
          )}
